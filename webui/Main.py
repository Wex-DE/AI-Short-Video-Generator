...
st.set_page_config(
    ...
    menu_items={
        "Report a bug": "https://github.com/Wex-DE/AI-Short-Video-Generator/issues",
        "About": "# WexAuto\nSimply provide a topic or keyword for a video, and it will "
        "automatically generate the video copy, video materials, video subtitles, "
        "and video background music before synthesizing a high-definition short "
        "video.\n\nhttps://github.com/Wex-DE/AI-Short-Video-Generator",
    },
)
...
def _render_brand(available_update: str | None = None):
    ...
    st.markdown(
        f"""
        <h1 class="mpt-brand">
            <span class="mpt-brand__name">WexAuto</span>
            <span class="mpt-brand__version" title="WexAuto Version">v{html.escape(str(config.project_version))}</span>
            {lic_badge}
            {update_link}
        </h1>
        """,
...
def _render_license_lockout(result: license_manager.LicenseCheckResult):
    """Renders full-screen security verification card and halts further execution."""
    st.markdown(
        """
        <style>
        .wex-lock-card {
    ...
    status_color = (
        ...
        if result.status
        in [
            license_manager.LicenseStatus.BANNED,
            license_manager.LicenseStatus.EXPIRED,
        ]
        else "#ffab00"
    )
    ...
    st.markdown(
        f"""
        <div class="wex-lock-card">
            <div style="font-size: 52px; margin-bottom: 12px;">🔒</div>
            <div class="wex-lock-title">WexAuto Device Authentication</div>
            <div class="wex-lock-desc">
                To access and use this application, your device must be authorized.<br/>
                Please copy your <b>Device Code</b> below and send it to your administrator for activation.
            </div>
            <div style="font-size: 12px; color: #888; text-transform: uppercase; letter-spacing: 1.5px;">Your Unique Device Code</div>
            <div class="wex-code-badge">{html.escape(result.device_id)}</div>
            <div style="margin-top: 10px; font-size: 13px; color: {status_color};">
                <b>Status:</b> {html.escape(result.message)}
            </div>
        </div>
        """,
        unsafe_allow_html=True,
